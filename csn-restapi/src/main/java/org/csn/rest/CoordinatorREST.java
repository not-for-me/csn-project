...
public class CoordinatorREST {
    ...
    public Response setCSNConfigMetadata(@QueryParam("action") String action, CSNConfiguration input) {
        if(action != null) {
            ...
            switch(action) {
                ...
                case "start":
                    if(coordinator == null) {
                        ...
                        this.setCurrentTime();
                        coordinator = CSNProvider.getCSNCoreService(input.getCsnName(), this.getConfiguration());
                        coordinator.initCSN();
            ...
            this.makeResponseData("POST");
            return Response.created(createdUri).entity(retJsonMap).type(MediaType.APPLICATION_JSON).build();
        }
    }
    ...
    public Response getCSNConfigMetadata() {
        ...
        if(configuration == null)
            ...
            return Response.ok(this.getConfiguration(), MediaType.APPLICATION_JSON).build();
        }
    }
    ...
    public Response getBrokerStatus() {
        Map<String, Object> jsonDataMap = new HashMap<String, Object>();
        jsonDataMap.put("status", coordinator.getMessageQueueManager().getCurrentHealthStatus());
        jsonDataMap.put("inNum", coordinator.getMessageQueueManager().getTotalMSGEnqueueCount() );
        jsonDataMap.put("outNum", coordinator.getMessageQueueManager().getTotalMSGDequeueCount());
        jsonDataMap.put("proNum", coordinator.getMessageQueueManager().getTotalProducerCount());
        jsonDataMap.put("conNum", coordinator.getMessageQueueManager().getTotalConsumerCount());
        jsonDataMap.put("storage", coordinator.getMessageQueueManager().getStoreUsagePercentage());
        jsonDataMap.put("mem", coordinator.getMessageQueueManager().getMemoryUsagePercentage());
