...
public class NetworkREST {
    ...
    public Response createNetwork(NetworkSeed input ) {
        ...
        String id = coordinator.getSensorNetworkManager().registerNetwork(input.getName().replaceAll("\\p{Space}", "").replace(".","_"), input.getMembers(), input.getTags());
        ...
        if(id != null) {
            ...
            SensorNetwork metadata = coordinator.getSensorNetworkManager().getNetwork(id);
    ...
    @GET
    @Produces(MediaType.APPLICATION_JSON)
    public Response getNetworkInfo(@QueryParam("select") String select, @QueryParam("index") int index, @QueryParam("num") int num ) {
        if(select == null ) {
            logger.info("Get All Network Data");
            Set<SensorNetwork> metaList = coordinator.getSensorNetworkManager().getAllNetworkResources();
            if (metaList != null) {
                try { logger.info("Data which is sent: {}", mapper.writeValueAsString(metaList)); } catch (JsonProcessingException e) { e.printStackTrace(); }
            ...
            switch (select) {
                ...
                case "topics":
                    ...
                    if(num > 0)
                        set = (select.equals("ids")) ?
                                coordinator.getSensorNetworkManager().getNetworkIDs(index, num)
                                : coordinator.getSensorNetworkManager().getNetworkTopicPaths(index, num);

                    else
                        set = (select.equals("ids")) ?
                                coordinator.getSensorNetworkManager().getAllNetworkIDs()
                                : coordinator.getSensorNetworkManager().getAllNetworkTopicPaths();
                ...
                case "members":
                    ...
                    Map<String, Set<String>> membersMap = coordinator.getSensorNetworkManager().getAllNetworkAndMemberIDs();
                    if(membersMap != null) {
                        try { logger.info("Data which is sent: {}", mapper.writeValueAsString(membersMap)); } catch (JsonProcessingException e) { e.printStackTrace(); }
                        return Response.ok(membersMap, MediaType.APPLICATION_JSON).build();
                    }
                case "counts":
                    logger.info("Network Count API");
                    int totalNetworkCNT = coordinator.getSensorNetworkManager().getAllNetworkCount();
                    int operatingNetworkCNT = coordinator.getSensorNetworkManager().getOperatingNetworkCount();
                    int stoppedNetworkCNT = coordinator.getSensorNetworkManager().getStoppedNetworkCount();
                    int faultedNetworkCNT = coordinator.getSensorNetworkManager().getFaultedNetworkCount();
                    ...
                    if(totalNetworkCNT > -1) {
                        ...
                        try { logger.info("Data which is sent: {}", mapper.writeValueAsString(dataMap)); } catch (JsonProcessingException e) { e.printStackTrace(); }
                        return Response.ok(dataMap, MediaType.APPLICATION_JSON).build();
                    }
            }
        }
    ...
    public Response removeAllNetwork() {
        ReturnType retType = coordinator.getSensorNetworkManager().deactivateAllNetworks();
    ...
    public Response getNetwork(@PathParam("id") String id) {
        ...
        if(network != null) {
            try { logger.info("Data which is sent: {}", mapper.writeValueAsString(network)); } catch (JsonProcessingException e) { e.printStackTrace(); }
    ...
    @Path("/{id}/parents")
    @Produces(MediaType.APPLICATION_JSON)
    public Response getParents(@PathParam("id") String id, @QueryParam("select") String select) {
        logger.info("Input Network ID: {}", id );
        Set set = null;
    ...
    public Response getMemberIDs(@PathParam("id") String id) {
        logger.info("Input  Network ID: {}", id );
        Set<String> memberSet = coordinator.getSensorNetworkManager().getMemberIDs(id);
        if(memberSet != null) {
            try { logger.info("Data which is sent: {}", mapper.writeValueAsString(memberSet)); } catch (JsonProcessingException e) { e.printStackTrace(); }
    ...
    public Response getTopicPath(@PathParam("id") String id) {
        logger.info("Input  Network ID: {}", id );
        String topicName = coordinator.getSensorNetworkManager().getTopicPath(id);
        if(topicName != null) {
...
//    @Consumes(MediaType.APPLICATION_JSON)
//    @Produces(MediaType.APPLICATION_JSON)
//    public Response setNetworkOptional(Map<String, String> input, @PathParam("id") String id) {
//        logger.info(input.toString());
//        logger.info("Input  Network ID: {}", id );
//        DAOReturnType retType = coordinator.getSensorNetworkManager().addOptionalNetwork(id, input);
//
//        jsonDataMap.put(JsonKeyName.METHOD_NAME,"POST");
//        jsonDataMap.put(JsonKeyName.SCOPE_NAME,"ONE");
//
...
//        Map<String, String> optMap = coordinator.getSensorNetworkManager().getAllOptionalNetwork(id);
//        if(optMap != null) {
//            try { logger.info("Data which is sent: {}", mapper.writeValueAsString(optMap)); } catch (JsonProcessingException e) { e.printStackTrace(); }
...
//    @Consumes(MediaType.APPLICATION_JSON)
//    @Produces(MediaType.APPLICATION_JSON)
//    public Response updateNetworkOptional(Map<String, String> input, @PathParam("id") String id) {
//        logger.info(input.toString());
//        logger.info("Input  ID: {}", id );
//        DAOReturnType retType = coordinator.getSensorNetworkManager().updateOptionalNetwork(id, input);
...
//        logger.info("Input  Network ID: {}", id );
//        DAOReturnType retType = coordinator.getSensorNetworkManager().removeOptionalNetwork(id);
//        jsonDataMap.put(JsonKeyName.METHOD_NAME,"DELETE");
//        jsonDataMap.put(JsonKeyName.SCOPE_NAME,"ALL");
//
//        if(retType != DAOReturnType.RETURN_ERROR) {
...
//    @Produces(MediaType.APPLICATION_JSON)
//    public Response getNetworkOptaionalValue(@PathParam("id") String id, @PathParam("opt_name") String opt_name) {
//        logger.info("Input  ID: {}", id );
//        logger.info("Input Option Name: {}", opt_name);
//        String opt_val = coordinator.getSensorNetworkManager().getOptionalNetworkValue(id, opt_name);
//        if(opt_val != null) {
...
//    @Consumes(MediaType.APPLICATION_JSON)
//    @Produces(MediaType.APPLICATION_JSON)
//    public Response updateNetworkOptionalValue(Map<String, String> input, @PathParam("id") String id, @PathParam("opt_name") String opt_name) {
//        logger.info(input.toString());
//        logger.info("Input  Network ID: {}", id );
//        logger.info("Input Option Name: {}", opt_name);
//        DAOReturnType retType = coordinator.getSensorNetworkManager().updateOptionalNetwork(id, input);
...
//    @Path("/{id}/opts/{opt_name}")
//    @Produces(MediaType.APPLICATION_JSON)
//    public Response removeNetworkOptionalValue(@PathParam("id") String id, @PathParam("opt_name") String opt_name) {
//        logger.info("Input  Network ID: {}", id );
//        logger.info("Input Option Name: {}", opt_name);
//        DAOReturnType retType = coordinator.getSensorNetworkManager().removeOptionalNetworkValue(id, opt_name);
//
//        jsonDataMap.put(JsonKeyName.METHOD_NAME,"DELETE");
//        jsonDataMap.put(JsonKeyName.SCOPE_NAME,"ONE");
//
...
//
    ...
    public Response addConcepts(Set<String> input,  @PathParam("id") String id) {
        logger.info(input.toString());
        logger.info("Input  Network ID: {}", id );
        ReturnType retType = coordinator.getSensorNetworkManager().addTags(input, id);
    ...
    public Response getAllConcepts(@PathParam("id") String id) {
        logger.info("Input  Network ID: {}", id );
        Set<String> tagSet = coordinator.getSensorNetworkManager().getAllTags(id);
        if(tagSet != null) {
    ...
    public Response removeNetworkAllTag(@PathParam("id") String id) {
        logger.info("Input  Network ID: {}", id );
        ReturnType retType = coordinator.getSensorNetworkManager().removeAllTags(id);
        jsonDataMap.put(JsonKeyName.METHOD_NAME,"DELETE");
        jsonDataMap.put(JsonKeyName.SCOPE_NAME,"ONE");
